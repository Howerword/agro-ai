...
export function ChatBot() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {open ? (
        <div className="mb-4 w-[380px] rounded-[28px] border border-stone-200 bg-white p-5 shadow-2xl">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-brand-700">AI Bot</p>
              <h3 className="text-lg font-semibold text-stone-900">Підбір добрив</h3>
            </div>
            <button className="text-sm text-stone-500" onClick={() => setOpen(false)} type="button">
              Закрити
            </button>
          </div>
          ...
          <div className="mt-4 max-h-72 space-y-3 overflow-y-auto rounded-3xl bg-stone-50 p-4">
          ...
          <div className="mt-4">
            <textarea
              ...
              onKeyDown={onKeyDown}
              className="min-h-24 w-full rounded-2xl border border-stone-200 px-4 py-3 text-sm"
              placeholder="Напишіть запит: що вирощуєте, яка задача, який симптом або фаза розвитку?"
            />
          </div>

          {suggestions.length ? (
            <div className="mt-4 rounded-2xl border border-stone-200 p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-brand-700">Рекомендовані продукти</p>
              <div className="mt-3 space-y-2">
                {suggestions.map((product) => (
                  <Link
                    key={product.id}
                    href={`/product/${product.slug}`}
                    className="block rounded-2xl bg-stone-50 px-3 py-3 text-sm text-stone-700 hover:bg-stone-100"
                  >
          ...
          {lastResponse?.leadCaptureRecommended ? (
            <p className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-800">
              Якщо хочете, залиште заявку через форму на сторінці, і менеджер підготує персональну схему.
            </p>
          ) : null}
          ...
          <button
            ...
            disabled={loading}
            onClick={() => void send()}
            className="mt-4 rounded-full bg-brand-700 px-5 py-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60"
          >
      ...
      <button
        ...
        className="mt-4 rounded-full bg-brand-700 px-5 py-3 text-sm font-medium text-white shadow-lg"
      >
        AI-консультант
      </button>
    </div>
